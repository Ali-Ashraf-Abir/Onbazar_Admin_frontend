...
export default function AdminGiftCardDetailPage() {
    ...
    async function load() {
      ...
      try {
        ...
        setSelectedTemplateId(
          typeof gc.templateId === "object" ? gc.templateId?._id : gc.templateId
        );
      } catch (err) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "var(--bw-bg)", color: "var(--bw-ink)" }}>
    ...
    <div className="min-h-screen" style={{ background: "var(--bw-bg)", color: "var(--bw-ink)", fontFamily: "var(--bw-font-body)" }}>
      <div className="max-w-[1000px] mx-auto px-4 sm:px-5 py-6 sm:py-8">
        <div className="flex items-center justify-between mb-6 gap-3">
          <div>
            <h1 className="text-2xl sm:text-3xl tracking-tight" style={{ fontFamily: "var(--bw-font-display)" }}>Edit Gift Card</h1>
            <p className="mt-0.5 text-sm" style={{ color: "var(--bw-muted)" }}>{giftCard?.name}</p>
          </div>
        ...
        {error && (
          <div className="mb-4 p-4 rounded-[var(--bw-radius-md)] text-sm" style={{ background: "rgba(220,38,38,0.15)", color: "rgb(220,38,38)", border: "1px solid rgba(220,38,38,0.3)" }}>
            {error}
          </div>
        )}
        {success && (
          <div className="mb-4 p-4 rounded-[var(--bw-radius-md)] text-sm" style={{ background: "rgba(22,163,74,0.15)", color: "rgb(22,163,74)", border: "1px solid rgba(22,163,74,0.3)" }}>
            ✓ {success}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-6">
          <div className="lg:col-span-2 flex flex-col gap-5 sm:gap-6">
            <div className="rounded-[var(--bw-radius-lg)] p-5 sm:p-6" style={{ background: "var(--bw-surface)", border: "1px solid var(--bw-border)", boxShadow: "var(--bw-shadow-sm)" }}>
              <h2 className="text-lg font-bold mb-5" style={{ fontFamily: "var(--bw-font-display)" }}>Basic Information</h2>
                ...
                <div>
                  ...
                  {thumbnailPreview ? (
                    <div className="relative rounded-[var(--bw-radius-md)] overflow-hidden border" style={{ borderColor: "var(--bw-border)" }}>
                      <img src={thumbnailPreview} alt="Cover" className="w-full h-44 object-cover" style={{ opacity: uploadingThumbnail ? 0.5 : 1 }} />
                      {!uploadingThumbnail && (
                        <label
                  ...
                  {thumbnailError && <p className="text-[11px] mt-1.5" style={{ color: "rgb(220,38,38)" }}>{thumbnailError}</p>}
                </div>

                <div>
                  <label className={labelCls}>Gift Card Name *</label>
                  <input className={inputBase} type="text" name="name" value={formData.name} onChange={handleInputChange} />
                </div>

                <div>
                  <label className={labelCls}>Description</label>
                  <textarea className={inputBase} name="description" value={formData.description} onChange={handleInputChange} rows={3} />
                </div>

                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className={labelCls}>Selling Price *</label>
                    <input className={inputBase} type="number" name="sellingPrice" value={formData.sellingPrice} onChange={handleInputChange} min="0" />
                  </div>
                  <div>
                    <label className={labelCls}>Cost Price</label>
                    <input className={inputBase} type="number" name="costPrice" value={formData.costPrice} onChange={handleInputChange} min="0" />
                  </div>
                  <div>
                    <label className={labelCls}>Currency</label>
                    <input className={inputBase} type="text" name="currency" value={formData.currency} onChange={handleInputChange} />
                  </div>
                </div>

                <div>
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input type="checkbox" name="isActive" checked={formData.isActive} onChange={handleInputChange} className="w-4 h-4 rounded cursor-pointer" />
            ...
            <div className="rounded-[var(--bw-radius-lg)] p-5 sm:p-6" style={{ background: "var(--bw-surface)", border: "1px solid var(--bw-border)", boxShadow: "var(--bw-shadow-sm)" }}>
              <h2 className="text-lg font-bold mb-5" style={{ fontFamily: "var(--bw-font-display)" }}>Template</h2>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-96 overflow-y-auto">
                {templates.map((template: any) => {
                  ...
                  return (
                    ...
                    >
                      {template.thumbnail ? (
                        <img src={template.thumbnail} alt={template.name} className="w-full h-24 object-cover rounded-[var(--bw-radius-sm)] mb-2" />
                      ) : (
                        <div className="w-full h-24 rounded-[var(--bw-radius-sm)] mb-2 flex items-center justify-center text-2xl" style={{ background: "var(--bw-bg)" }}>🖼️</div>
                      )}
              ...
              <p className="text-[10px] mt-3" style={{ color: "var(--bw-ghost)" }}>
                Changing the template only affects future orders — orders already placed keep the template
                that was live when the customer designed their card (frozen as `giftCardTemplateId`).
              </p>
            </div>
          </div>
