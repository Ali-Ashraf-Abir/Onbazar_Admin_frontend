...
function toCloudinaryDownloadUrl(url: string, filename?: string) {
  if (!url.includes("res.cloudinary.com")) return url; // fallback: leave non-Cloudinary URLs untouched
...
export default function AdminOrderDetailPage() {
  ...
  function handleStatusChange(next: string) {
    if (REFUND_STATUSES.has(next) && !REFUND_STATUSES.has(order?.status)) {
  ...
  function handleRefundSuccess(updated: any) {
    setOrder(updated);
  ...
  function handleRefundClose() {
    setPendingStatus(null);   // select snaps back to current committed status
    setShowRefundModal(false);
  }
  ...
  const delivery = order.pricing?.deliveryCharge ?? null;

  // Show pending status in select while modal is open so it
  // doesn't snap back before the admin confirms or cancels.
  const selectValue = showRefundModal ? (pendingStatus ?? newStatus) : newStatus;
    ...
    <>
      ...
      >
        <div className="max-w-[1200px] mx-auto px-6 py-8">
          ...
          {/* ── Refund banner — visible once a refund has been issued ── */}
          {order.refund && (
            <div
              className="mb-5 rounded-[var(--bw-radius-lg)] px-5 py-4 flex items-start justify-between gap-4"
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="text-[13px] font-bold" style={{ color: "#dc2626" }}>
                    {order.status === "cancelled" ? "Order Cancelled" : "Refund Issued"}
                  </div>
                  <div className="text-[12px] mt-0.5" style={{ color: "var(--bw-muted)" }}>
                    {order.refund.refundedAmount != null && (
                      <>
                        <span className="font-semibold" style={{ color: "var(--bw-ink)" }}>
                          {fmt(order.refund.refundedAmount, cur)}
                        </span>{" "}refunded
                        {order.refund.refundedAmount < order.pricing?.subtotal
                          ? <> · <span className="font-semibold" style={{ color: "var(--bw-green)" }}>
                            {fmt(order.pricing.subtotal - order.refund.refundedAmount, cur)}
                          </span> kept (partial)</>
                          : " (full)"
                        }
                      </>
                    )}
                    {order.refund.refundedAt && <> · {fmtDate(order.refund.refundedAt)}</>}
                  </div>
                  {order.refund.note && (
                    <div className="text-[12px] mt-1 italic" style={{ color: "var(--bw-muted)" }}>
                      "{order.refund.note}"
                    </div>
                  )}
                  {order.refund.items?.length > 0 && (
                    <div className="text-[11px] mt-1.5 flex flex-wrap gap-1.5">
                      {order.refund.items.map((ri: any, i: number) => {
                        const matched = order.items?.find((it: any) =>
                          (it.product?._id ?? it.product)?.toString() === ri.product?.toString()
                          && it.size === ri.size
                        );
                        return (
                          <span
                          ...
                          >
                            {matched?.snapshot?.name || "Item"}{ri.size ? ` (${ri.size})` : ""} × {ri.quantity}
                          </span>
                        );
                      })}
                    </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
            {[
              ...
              { label: "Delivery Charge", value: delivery != null ? fmt(delivery, cur) : "Not set", sub: "Delivery charges", special: delivery == null ? "na" : null },
