...
export default function AdminKeyringsPage() {
  ...
  const [applied, setApplied] = useState({
    ...
    minPrice: "", maxPrice: "", keyringType: "",
    category: "", isBestProduct: "", page: 1,
  });

  /* ─────────────────────── fetch ─────────────────────── */

  async function loadKeyrings(a: typeof applied) {
    setLoading(true);
    try {
      ...
      if (a.keyringType)   params.set("keyringType",  a.keyringType);
      ...
      const data = await api.get<any>(`/keyrings?${params.toString()}`);
      setKeyrings(data.data || []);
      setMeta(data.meta || null);
    } catch { /* silent */ }
    finally { setLoading(false); }
  }

  useEffect(() => { loadKeyrings(applied); }, [applied]);
  ...
  function applyFilters() {
    setApplied({ q, isActive, sort, minPrice, maxPrice, keyringType, category, isBestProduct, page: 1 });
  ...
  function resetFilters() {
    const d = {
      ...
      maxPrice: "", keyringType: "", category: "", isBestProduct: "", page: 1,
    };
    setQ(""); setIsActive("true"); setSort("newest"); setMinPrice("");
    setMaxPrice(""); setKeyringType(""); setCategory("");
  ...
  const hasActiveFilters = !!(
    ...
    applied.keyringType || applied.category ||
  ...
  const hasPendingChanges =
    ...
    keyringType !== applied.keyringType || category !== applied.category ||
    isBestProduct !== applied.isBestProduct;
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-5 py-6 sm:py-8">
          ...
          <div>
            ...
            {meta && (
              <p className="mt-0.5 text-sm" style={{ color: "var(--bw-muted)" }}>
                {meta.total ?? keyrings.length} keyring{(meta.total ?? keyrings.length) !== 1 ? "s" : ""}
                {meta.totalPages > 1 && ` · Page ${meta.page} of ${meta.totalPages}`}
              </p>
            )}
            <div className="mt-2">
              <a href="/admin/keyrings/create" className="px-4 py-2 bg-[var(--bw-ink)] text-[var(--bw-bg)] rounded-md text-sm font-medium">
                Create Keyring
              </a>
            </div>
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
            ...
            <div>
              ...
              <input className={inputBase} value={minPrice} onChange={(e) => setMinPrice(e.target.value)} placeholder="৳ 0" type="number" min="0" />
            </div>

            <div>
              <label className={labelCls}>Max Price</label>
              <input className={inputBase} value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} placeholder="৳ any" type="number" min="0" />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 pt-1 border-t" style={{ borderColor: "var(--bw-divider)" }}>
            ...
            >
              {hasPendingChanges && (
                <span className="w-2 h-2 rounded-full shrink-0" style={{ background: "var(--bw-bg)", animation: "bw-pulse 1.2s ease-in-out infinite" }} />
              )}
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          {applied.isBestProduct === "true" && (
            <FilterChip
              label="⭐ Best Products only"
              onRemove={() => { setIsBestProduct(""); setApplied((a) => ({ ...a, isBestProduct: "", page: 1 })); }}
            />
          )}
          {applied.keyringType && (
            <FilterChip
              label={`Type: ${applied.keyringType}`}
              onRemove={() => { setKeyringType(""); setApplied((a) => ({ ...a, keyringType: "", page: 1 })); }}
            />
          )}
        </div>
          ...
          {loading ? (
            ...
            <div className="col-span-full py-16 sm:py-20 text-center">
              ...
              <p className="text-sm" style={{ color: "var(--bw-ghost)" }}>
                Try adjusting your filters or create a new keyring.
              </p>
            </div>
          ) : (
            keyrings.map((k: any) => {
              const sp = k.pricing?.sellingPrice ?? 0;
              const currency = k.pricing?.currency ?? "BDT";
              const ep = effectivePrice(k);
              const cost = totalCost(k);
              const mgn = margin(k);
              const mgnPct = marginPct(k);
              const hasDiscount = k.discount && ep < sp;

              return (
                <a
                  key={k._id}
                  href={`/admin/keyrings/${k._id}`}
                ...
                >
                  ...
                  <div className="relative overflow-hidden" style={{ background: "var(--bw-surface-alt)" }}>
                    {k.images?.[0] ? (
                      ...
                      <div className="w-full h-40 sm:h-48 flex items-center justify-center text-4xl opacity-20" style={{ color: "var(--bw-ghost)" }}>
                        🔑
                      </div>
                    )}
                    ...
                    <div className="absolute top-2 right-2 flex flex-col items-end gap-1">
                      ...
                      >
                        {k.isActive ? "Active" : "Draft"}
                      </span>

                      {k.isBestProduct && (
                        <span className="text-[10px] font-bold tracking-wide px-2 py-0.5 rounded-full" style={{ background: "rgba(234,179,8,0.92)", color: "#713f12" }}>
                          ⭐ Best
                        </span>
                      )}

                      {k.keyringType && (
                        <span className="text-[10px] font-bold tracking-wide px-2 py-0.5 rounded-full" style={{ background: "rgba(79,70,229,0.85)", color: "#fff" }}>
                          {k.keyringType}
                        </span>
                      )}

                      {hasDiscount && (
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: "rgba(220,38,38,0.88)", color: "#fff" }}>
                          {k.discount.type === "percentage"
                            ? `${k.discount.value}% OFF`
                            : `${fmtPrice(k.discount.value, currency)} OFF`}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="p-3 sm:p-4 flex flex-col gap-1.5 flex-1">
                    {k.category?.name && (
                      <span
                        ...
                        style={{ color: "var(--bw-ink-secondary)", background: "var(--bw-surface-alt)", border: "1px solid var(--bw-border)" }}
                      >
                        {k.category.name}
                      </span>
                    )}
                    ...
                    {k.colors?.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {k.colors.slice(0, 5).map((c: string) => (
                          <span
                            key={c}
                        ...
                        {k.colors.length > 5 && (
                          <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: "var(--bw-bg-alt)", color: "var(--bw-muted)" }}>
                            +{k.colors.length - 5}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="mt-2 pt-2.5 flex flex-col gap-1.5" style={{ borderTop: "1px solid var(--bw-border)" }}>
                      <div className="flex justify-between items-baseline">
                        ...
                        <span className="flex items-baseline gap-1">
                          <span className="text-base font-bold tracking-tight" style={{ color: "var(--bw-ink)" }}>{fmtPrice(ep, currency)}</span>
                          {hasDiscount && (
                            <span className="text-[11px] line-through" style={{ color: "var(--bw-ghost)" }}>{fmtPrice(sp, currency)}</span>
                          )}
                        </span>
                      </div>
                        ...
                        <div className="flex items-center gap-1">
                          {mgn == null ? (
                            ...
                            <>
                              ...
                              {mgnPct && <span className="text-[10px]" style={{ color: "var(--bw-ghost)" }}>({mgnPct}%)</span>}
                            </>
                          )}
                        </div>
                      </div>
                      ...
                      {k.purchaseCount > 0 && (
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-[11px] font-semibold" style={{ color: "var(--bw-muted)" }}>
                            {k.purchaseCount} unit{k.purchaseCount !== 1 ? "s" : ""}
                          </span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-1.5">
                      <span className="text-[10px] truncate max-w-[100px] sm:max-w-[120px]" style={{ color: "var(--bw-ghost)", fontFamily: "var(--bw-font-mono)" }}>
                        {k.slug}
                      </span>
            ...
            {Array.from({ length: meta.totalPages }, (_, i) => i + 1)
              ...
              .map((p, i) =>
                p === "..." ? (
                  ...
                  <PageBtn key={p} onClick={() => goToPage(p as number)} disabled={false} label={String(p)} current={p === page} />
                )
              )}
